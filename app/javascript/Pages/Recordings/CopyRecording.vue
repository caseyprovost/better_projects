...
export default {
  ...
  data() {
    return {
      sending: false,
      form: {
        destination_bucket_id: this.copy.destination_bucket_id,
      },
    }
  },
  computed: {
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.post(this.$routes.bucket_recording_copies(this.currentBucket, this.recording), this.form).then(() => {
        this.sending = false
